...
def non_max_suppression(boxes, scores, threshold):
    """非最大値抑制を実行し、保持するボックスのインデックスを返します。
    boxes: [N, (y1, x1, y2, x2)]。(y2, x2)はボックスの外側にあることに注意。
    scores: ボックススコアの1次元配列。
    threshold: フィルタリングに使用するIoU閾値（Float）。
    """
    ...
    # スコアでソートされたボックスのインデックスを取得（最高スコアが最初）
    ixs = scores.argsort()[::-1]

    pick = []
    while len(ixs) > 0:
        # 最上位のボックスを選択し、そのインデックスをリストに追加
        i = ixs[0]
        pick.append(i)
        # 選択したボックスと残りのボックスとのIoUを計算
        iou = compute_iou(boxes[i], boxes[ixs[1:]], area[i], area[ixs[1:]])
        # 閾値を超えるIoUを持つボックスを特定。
        # これはixs[1:]へのインデックスを返すため、
        # ixsへのインデックスを取得するには1を加える。
        remove_ixs = np.where(iou > threshold)[0] + 1
        # 選択されたボックスと重複するボックスのインデックスを削除
